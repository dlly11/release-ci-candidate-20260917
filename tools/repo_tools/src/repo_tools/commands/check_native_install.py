...
COMPONENTS = {
    Path("include/release_candidate/core_version.h"): "RELEASE_CANDIDATE_CORE",
    Path("include/release_candidate/package_a_version.h"): "RELEASE_CANDIDATE_PACKAGE_A",
    Path("include/release_candidate/package_b_version.h"): "RELEASE_CANDIDATE_PACKAGE_B",
    Path("include/release_candidate/package_a_cli_version.h"): "RELEASE_CANDIDATE_PACKAGE_A_CLI",
}
...
def cli_errors(prefix: Path, version: str) -> list[str]:
    ...
    executable = (
        ...
        / "bin"
        / (
            "release-candidate-package-a-cli.exe"
            if os.name == "nt"
            else "release-candidate-package-a-cli"
        )
    )
    ...
    for argument, expected in (
        ("--version", f"release-candidate-package-a-cli {version}\n"),
        ("Ada", "Hello, Ada!\n"),
    ):
        try:
