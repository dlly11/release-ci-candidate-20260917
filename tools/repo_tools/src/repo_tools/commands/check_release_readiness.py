...
def ready(
    ...
    *,
    ...
    if automatic:
        ...
        sha = run["head_sha"]
        kind = "repository_dispatch" if run.get("event") == "repository_dispatch" else "push"
        check_run(run, repository, sha, workflow_id, event=kind)
    ...
    if automatic and event["workflow_run"].get("event") == "repository_dispatch":
