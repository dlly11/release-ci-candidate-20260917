...
def test_bad_artifacts_fail(tmp_path: Path, problem: str) -> None:
    ...
    if problem == "version":
        write_wheel(tmp_path, "release-candidate-core", version="0.0.1")
    elif problem == "unexpected":
        write_wheel(tmp_path, "another-package")
    elif problem == "duplicate":
        wheel = write_wheel(tmp_path, "release-candidate-core")
        shutil.copyfile(wheel, tmp_path / "duplicate.whl")
    with pytest.raises(ValueError):
        module.collect_wheels(tmp_path, {"release-candidate-core": "1.2.3"})
...
def test_install_uses_only_declared_dependencies(
    ...
    module = check_python_install
    core = write_wheel(tmp_path, "release-candidate-core", code='message = "Hello"\n')
    package = write_wheel(
        tmp_path,
        "release-candidate-package-a",
        requires="Requires-Dist: release-candidate-core>=1.0\n" if declared else "",
        code="from release_candidate_core import message\n",
    )
    constraints = tmp_path / "constraints.txt"
    constraints.write_text(f"release-candidate-core @ {core.as_uri()}\n", encoding="utf-8")
    monkeypatch.setitem(
        module.SMOKE_CHECKS,
        "release-candidate-package-a",
        SmokeCheck(
            "release_candidate_package_a",
            """
from release_candidate_package_a import message
...
""",
    ...
    (tmp_path / "release_candidate_core").mkdir()
    (tmp_path / "release_candidate_core/__init__.py").write_text(
        'message = "Hello"\n', encoding="utf-8"
    )
    monkeypatch.setenv("PYTHONPATH", str(tmp_path))
    if declared:
        module.check_wheel(
            uv, "release-candidate-package-a", "1.2.3", package, constraints, tmp_path
        )
    else:
...
def test_release_mode_checks_supplied_wheels_without_rebuilding(
    ...
    monkeypatch.setattr(
        module,
        "python_projects",
        lambda root: {Path("core/pyproject.toml"): "release-candidate-core"},
    )
    monkeypatch.setattr(
        module, "project_metadata", lambda root, path: ("release-candidate-core", "1.2.3")
    )
    monkeypatch.setattr(
        module, "SMOKE_CHECKS", {"release-candidate-core": SmokeCheck("release_candidate_core", "")}
    )
    monkeypatch.setattr(module.shutil, "which", lambda _: "uv")
...
def test_install_discovers_selected_project_configuration(
    ...
    monkeypatch.setitem(
        ...
        "release-candidate-core",
        SmokeCheck(
            "release_candidate_core",
            """
from release_candidate_core import message
...
""",
    ...
    monkeypatch.setenv("UV_CACHE_DIR", str(tmp_path / "cache"))
    module.check_wheel(
        uv, "release-candidate-core", "1.2.3", wheel, constraints, temporary, project
    )
...
def test_renamed_distribution_and_cli_retain_registered_checks(
    ...
    if uv is None:
        pytest.skip("uv is needed for the offline installation regression")
    smoke = SMOKE_CHECKS["release-candidate-package-a-cli"]
    monkeypatch.setitem(
