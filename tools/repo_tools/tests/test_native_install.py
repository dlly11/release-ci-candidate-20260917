...
def test_invalid_version_header(
    ...
    header = installation / "include/release_candidate/core_version.h"
    if defect == "missing_header":
        header.unlink()
    else:
        contents = header.read_text(encoding="utf-8")
        if defect == "missing_macro":
            contents = contents.replace("#define RELEASE_CANDIDATE_CORE_VERSION_MAJOR 1\n", "")
...
def test_cli_failures(
    ...
    def run(command: list[str], **kwargs: object) -> subprocess.CompletedProcess[str]:
        ...
        output = (
            "release-candidate-package-a-cli 1.2.3\n"
