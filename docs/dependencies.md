...
such as `release-candidate-core>=0.1.0` describes the oldest compatible API, not the current release.
...
uv resolves the workspace together; selecting `uv run --package release-candidate-package-a` does not create
...
```text
uv lock
uv sync --locked --all-packages --reinstall-package release-candidate-core --reinstall-package release-candidate-package-a --reinstall-package release-candidate-package-b --reinstall-package release-candidate-package-a-cli --reinstall-package monorepo-repo-tools
...
```bash
uv add --package release-candidate-package-a 'httpx>=0.28,<1'
uv add --group docs 'sphinx>=9.1,<10'
uv tree --package release-candidate-package-a
