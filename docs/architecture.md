...
| --- | --- | --- |
| Core | `release-candidate-core` | `release_candidate_core` |
| Package A | `release-candidate-package-a` | `release_candidate_package_a` |
| Package B | `release-candidate-package-b` | `release_candidate_package_b` |
| Package A CLI | `release-candidate-package-a-cli` | `release_candidate_package_a_cli` |
...
such as `<release_candidate/package_a.h>` and `<release_candidate/package_a_version.h>`. Consumers link through
namespaced CMake aliases:

```cmake
target_link_libraries(my_target PRIVATE release_candidate::package_a)
```
...
6. Add install rules for the library, public headers, and the shared `ReleaseCandidateTargets`
...
Downstream CMake projects use `find_package(ReleaseCandidate CONFIG REQUIRED)` and link the
`release_candidate::core`, `release_candidate::package_a`, or `release_candidate::package_b` targets. The install tree can be
moved without changing its generated CMake files.

Package configuration is installed beneath `<libdir>/cmake/ReleaseCandidate`. Consumers that
previously set `ReleaseCandidate_DIR` directly to `<libdir>/cmake/monorepo-template` must update
