...
`release-candidate-package-a-cli` exposes the greeting service as the `release-candidate-package-a-cli` command. The
...
```console
$ uv run release-candidate-package-a-cli "Ada Lovelace"
Hello, Ada Lovelace!
```
...
```console
$ uv run release-candidate-package-a-cli --prefix Welcome "Grace Hopper"
...
```bash
uv run python -m release_candidate_package_a_cli Ada
```
...
```{automodule} release_candidate_package_a_cli.cli
:members:
```
